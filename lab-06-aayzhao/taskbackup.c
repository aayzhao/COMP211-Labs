...
unsigned int set_task(unsigned int index, unsigned int pid) 
{
    ...
    // find target and its index
    task_struct* target = head;
    ...
    if (target == NULL) return TASK_OP_ERROR; // PID not found
    if (cur_idx == index) return TASK_OP_PASS; // index is same as current index

    // remove target task from list
    if (cur_idx == 0) head = head->next; // task is head of list
    if (cur_idx == node_num - 1) {
        tail = prev;  // task is tail of list
        tail->next = NULL;
        target->next = NULL;
    }
    else 
...
task_struct* remove_task(unsigned int pid) 
{
    ...
    if (head == tail) // if only 1 task in list
    {
